...
public class DrivebaseSimFX {
	...
	private final int kCountsPerRev = (int) Drivetrain.TICKS_PER_REV;  //Encoder counts per revolution of the motor shaft.
	...
	private final double kGearRatio = Drivetrain.GEAR_RATIO; //Switch kSensorGearRatio to this gear ratio if encoder is on the motor instead of on the gearbox.
	private final double kWheelDiameterMeters = Drivetrain.WHEEL_DIAMETER_METERS;
	...
	private DifferentialDrivetrainSim _driveSim = new DifferentialDrivetrainSim(
		...
		kWheelDiameterMeters/2.0,           //Robot uses 3" radius (6" diameter) wheels.
		Drivetrain.WHEEL_BASE_METERS,       //Distance between wheels is _ meters.
	...
	private int distanceToNativeUnits(double positionMeters){
		double wheelRotations = positionMeters/(Math.PI * kWheelDiameterMeters);
	...
	private int velocityToNativeUnits(double velocityMetersPerSecond){
		double wheelRotationsPerSecond = velocityMetersPerSecond/(Math.PI * kWheelDiameterMeters);
